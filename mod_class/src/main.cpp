...
#define BENCHMARK_(expr, desc) \
	{ \
		...
		long diff = 1000000000 * (end.tv_sec - start.tv_sec); \
		printf(desc "\ttook %ld nsec or %8lf sec\n", end.tv_nsec - start.tv_nsec + diff, (end.tv_nsec - start.tv_nsec + diff)/1000000000.0); \
	}
...
void test_mul() {
	...
	assert(Mod(1234567890)*987654321 == 1219326311126352690);
...
void test_pwr() {
	...
	assert(Mod(8133226969400185297).pwr(4429837672210038469) == 6370735146449132574);
	
	Mod::set_modulus(443);
	assert(Mod(443-1).pwr(443-1) == 1);

	assert(Mod(0).pwr(0) == 1);
	assert(Mod(2).pwr(0) == 1);
...
int main() {
    ...
    for (int i = 0; i < 100000; ++i)
	    Mod(7182237390125665823).pwr(8182237390125665823);
    ,"100,000 Mod().pwr() calls") 
#endif
    return 0;
}
